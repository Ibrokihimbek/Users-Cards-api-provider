...
import 'package:provider_example/bloc/cards_bloc.dart';
...
class UsersCardsBlockPage extends StatelessWidget {
  const UsersCardsBlockPage({super.key});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: BlocProvider(
        create: (context) =>
            CardsBloc(appRepository: context.read<AppRepository>())
              ..add(FetchAllCards()),
        child: BlocBuilder<CardsBloc, CardsBlocState>(
          builder: (context, state) {
            return Scaffold(
              ...
              body: state is LoadCardsBlocInProgress
                  ? const Center(child: ShimmerWidget())
                  : state is LoadCardsBlocInSuccess
                      ? Center(
                          child: Expanded(
                            child: ListView.builder(
                              shrinkWrap: true,
                              physics: const BouncingScrollPhysics(),
                              itemCount: state.cardsBloc.length,
                                ...
                                return Container(
                                  ...
                                  decoration: BoxDecoration(
                                    gradient: LinearGradient(
                                        ...
                                        colors: [
                                          Color(
                                            hexColor(state.cardsBloc[index]
                                                .colors.color_a),
                                          ),
                                          Color(
                                            hexColor(state.cardsBloc[index]
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          Text(
                                            state.cardsBloc[index].card_type,
                                          ...
                                          SizedBox(
                                            ...
                                            height: 40,
                                            child: Image.network(
                                              state.cardsBloc[index].icon_image,
                                            ),
                                          )
                                        ],
                                      ),
                                      ...
                                      Text(
                                        state.cardsBloc[index].bank_name,
                                        style: const TextStyle(
                                          fontSize: 16,
                                          color: Colors.white,
                                        ),
                                      ...
                                      Row(
                                        children: [
                                          Text(
                                            '${state.cardsBloc[index].money_amount}',
                                            style: const TextStyle(
                                              fontSize: 20,
                                      ...
                                      Text(
                                        state.cardsBloc[index].card_number,
                                        style: const TextStyle(
                                          fontSize: 18,
                                          fontWeight: FontWeight.w400,
                                          color: Colors.white,
                      ...
                      : state is LoadCardsBlocInFailure
                          ? Center(
                              child: Text(
